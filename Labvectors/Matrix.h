...
public:
    ...
    template <class B> friend istream& operator>>(istream& in, Vector<B>& V);
    template <class B> friend ostream& operator<<(ostream& out, Vector<B>& V);
...
istream& operator>>(istream& in, Vector<T>& V)
{
    ...
    in >> len;
    cout << "Vvedite hleni vectora: " << endl;
    V = Vector<T>(len);
    for (int i = 0; i < len; ++i)
        in >> V.data[i];
    return in;
...
public:
    ...
    template <class B> friend istream& operator>>(istream& in, Matrix<B>& M);
    template <class B> friend ostream& operator<<(ostream& out, Matrix<B>& M);
...
{
    cout << "Vvedite kollihestvo strok" << endl;
    int n, m;
    in >> n;
    cout << "Vvedite kollihestvo stolbcov" << endl;
    in >> m;
    cout << "Vvedite hleni matrici" << endl;
    M = Matrix<T>(n, m);
    for (int i = 0;i < n;++i)
        for (int j = 0;j < m;++j)
            in >> M.data[i][j];
